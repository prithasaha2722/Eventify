...
def make_certificates(name,event,date,venue,desig,design,n1,n2):
    ...
    draw.text((720,546), name, fill=FONT_COLOR)
    event_width, event_height = draw.textsize(event)
    draw.text((406,870), event, fill=FONT_COLOR)
    date_width, date_height = draw.textsize(date)
    draw.text((1212,869), date, fill=FONT_COLOR)
    venue_width, venue_height = draw.textsize(venue)
    draw.text((739,867), venue, fill=FONT_COLOR)
