...
def make_certificates(name,gender,field,desig,design,n1,n2):
    ...
    draw.text((762,568), name, fill=FONT_COLOR)
    gender_width, gender_height = draw.textsize(gender)
    draw.text((1000,792), gender, fill=FONT_COLOR)
    gender_width, gender_height = draw.textsize(gender)
    draw.text((1229,853), gender, fill=FONT_COLOR)
    field_width, field_height = draw.textsize(field)
    draw.text((579,851), field, fill=FONT_COLOR)
    desig_width, desig_height = draw.textsize(desig)
    draw.text((520,1191), desig, fill=FONT_COLOR)
    design_width, design_height = draw.textsize(design)
    draw.text((1322,1197), design, fill=FONT_COLOR)
    n1_width, n1_height = draw.textsize(n1)
    draw.text((488,1079), n1, fill=FONT_COLOR)
    n2_width, n2_height = draw.textsize(n2)
    draw.text((1301,1081), n2, fill=FONT_COLOR)
